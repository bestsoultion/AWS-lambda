...
class DynamoCountRepository(CountRepository, SQSManager, SnsNotifier):
    ...
    def get_count_by_count_date_range(self, supplier_id, min_count_date, max_count_date=None):
        ...
        if max_count_date:
            query = {
                'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('count_date').between(
                    min_count_date,
                    max_count_date),
                'FilterExpression':
                    Attr('latest').eq(True) & Attr('active').eq(True),
            ...
            query = {
                'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('count_date').gt(
                    min_count_date),
                'FilterExpression':
                    Attr('latest').eq(True) & Attr('active').eq(True),
                'IndexName': 'by_supplier_id_and_count_date'
    ...
    def save_count(self, obj):
        ...
        count_obj = self._storage.save(table, obj)
