...
class DynamoPurchaseOrderRepository(PurchaseOrderRepository, SQSManager, SnsNotifier):
    ...
    def get_purchase_orders_by_order_date_range(self, min_order_date, max_order_date=None,
        ...
        if supplier_id:
            if max_order_date:
                query = {
                    'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('order_date').between(
                        min_order_date,
                        max_order_date),
                    'FilterExpression':
                        Attr('latest').eq(True) & Attr('active').eq(True),
                    'IndexName': 'by_supplier_id_and_order_date'
                }
            else:
                query = {
                    'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('order_date').gt(
                        min_order_date),
                    'FilterExpression':
                        Attr('latest').eq(True) & Attr('active').eq(True),
                    'IndexName': 'by_supplier_id_and_order_date'
            ...
            for distributor in distributors:
                if max_order_date:
                    query = {
                        'KeyConditionExpression': Key('distributor_id').eq(distributor) & Key('order_date').between(
                            min_order_date,
                            max_order_date),
                        'FilterExpression':
                            Attr('latest').eq(True) & Attr('active').eq(True),
                    ...
                    query = {
                        'KeyConditionExpression': Key('distributor_id').eq(distributor) & Key('order_date').gt(
                            min_order_date),
                        'FilterExpression':
                            Attr('latest').eq(True) & Attr('active').eq(True),
                        'IndexName': 'by_distributor_id_and_order_date'
    ...
    def get_purchase_orders_by_pack_date_range(self, min_pack_date, max_pack_date=None,
        ...
        if supplier_id:
            if max_pack_date:
                query = {
                    'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('pack_date').between(
                        min_pack_date,
                        max_pack_date),
                    'FilterExpression':
                        Attr('latest').eq(True) & Attr('active').eq(True),
                    'IndexName': 'by_supplier_id_and_pack_date'
                }
            else:
                query = {
                    'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('pack_date').gt(
                        min_pack_date),
                    'FilterExpression':
                        Attr('latest').eq(True) & Attr('active').eq(True),
                    'IndexName': 'by_supplier_id_and_pack_date'
            ...
            for distributor in distributors:
                if max_pack_date:
                    query = {
                        'KeyConditionExpression': Key('distributor_id').eq(distributor) & Key('pack_date').between(
                            min_pack_date,
                            max_pack_date),
                        'FilterExpression':
                            Attr('latest').eq(True) & Attr('active').eq(True),
                    ...
                    query = {
                        'KeyConditionExpression': Key('distributor_id').eq(distributor) & Key('pack_date').gt(
                            min_pack_date),
                        'FilterExpression':
                            Attr('latest').eq(True) & Attr('active').eq(True),
                        'IndexName': 'by_distributor_id_and_pack_date'
    ...
    def get_purchase_orders_by_ship_date_range(self, min_ship_date, max_ship_date=None,
        ...
        if supplier_id:
            if max_ship_date:
                query = {
                    'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('ship_date').between(
                        min_ship_date,
                        max_ship_date),
                    'FilterExpression':
                        Attr('latest').eq(True) & Attr('active').eq(True),
                    'IndexName': 'by_supplier_id_and_ship_date'
                }
            else:
                query = {
                    'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('ship_date').gt(
                        min_ship_date),
                    'FilterExpression':
                        Attr('latest').eq(True) & Attr('active').eq(True),
                    'IndexName': 'by_supplier_id_and_ship_date'
            ...
            for distributor in distributors:
                if max_ship_date:
                    query = {
                        'KeyConditionExpression': Key('distributor_id').eq(distributor) & Key('ship_date').between(
                            min_ship_date,
                            max_ship_date),
                        'FilterExpression':
                            Attr('latest').eq(True) & Attr('active').eq(True),
                    ...
                    query = {
                        'KeyConditionExpression': Key('distributor_id').eq(distributor) & Key('ship_date').gt(
                            min_ship_date),
                        'FilterExpression':
                            Attr('latest').eq(True) & Attr('active').eq(True),
                        'IndexName': 'by_distributor_id_and_ship_date'
