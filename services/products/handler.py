...
def get_every_product(event, context):
    """
    Get all product
    """
    ...
    items = context.repo.get_all_products(supplier)
...
def add_product(event, context):
    ...
    try:
        body = context.repo.save_product(body)
        return {
            'statusCode': 200,
...
def modify_product(event, context):
    ...
    try:
        item = context.repo.save_product(body)
...
def get_by_id(event, context):
    ...
    try:
        ...
        item = context.repo.get_product_by_id(context.supplier_id, entity_id)
...
def delete_by_id(event, context):
    """
    Delete product by entity_id and version
    """
    logger.debug('event: {}'.format(event))
    ...
    try:
        ...
        context.repo.delete_product_by_id(context.supplier_id, entity_id)
        return {
            'statusCode': 204
        }
    except NoSuchEntity:
