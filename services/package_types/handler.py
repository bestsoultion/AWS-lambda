...
def get_every_package_type(event, context):
    """
    Get all package_type
    """
    ...
    items = context.repo.get_all_package_types(supplier)
...
@insert_repo
@check_supplier
def add_package_type(event, context):
    """
    Add a new package_type
    """
    logger.debug('event: {}'.format(event))
    ...
    try:
        body = context.repo.save_package_type(body)
        return {
            'statusCode': 200,
...
@insert_repo
@check_supplier
def modify_package_type(event, context):
    logger.debug('event: {}'.format(event))
    ...
    try:
        item = context.repo.save_package_type(body)
...
def get_by_id(event, context):
    ...
    try:
        ...
        item = context.repo.get_package_type_by_id(context.supplier_id, entity_id)
...
def delete_by_id(event, context):
    """
    Delete package_type by entity_id and version
    """
    logger.debug('event: {}'.format(event))
    ...
    try:
        ...
        context.repo.delete_package_type_by_id(context.supplier_id, entity_id)
        return {
            'statusCode': 204
        }
    except NoSuchEntity:
