...
def create_base_message_data(order_data):
    ...
    if "order_date" in order_data:
        t = datetime.utcfromtimestamp(order_data['order_date'])
        message_data['order_date'] = t.strftime("%m-%d-%Y")

    if "pack_date" in order_data:
        t = datetime.utcfromtimestamp(order_data['pack_date'])
        message_data['pack_date'] = t.strftime("%m-%d-%Y")

    if "ship_date" in order_data:
        t = datetime.utcfromtimestamp(order_data['ship_date'])
        message_data['ship_date'] = t.strftime("%m-%d-%Y")
    ...
    if "number_of_pallets" in order_data:
        message_data["pallets"] = order_data["number_of_pallets"]
    else:
...
def get_product_row(product, product_status):
    row_text = ""
...
def create_product_table(order_data, previous_order_data):
    ...
    for brand in brand_list:
        ...
        for product in brand_products:
            ...
            table_text += get_product_row(product, product_status)
...
def create_merchandise_table(order_data, previous_order_data):
    ...
    table_text = "<table width=100%><tr><td colspan=20><b>Merchandise</b></td></tr>"
...
def check_for_updated_data(order_data, previous_order_data, message_data, property_name):
    if property_name in order_data and  property_name in previous_order_data:
        if order_data[property_name] != previous_order_data[property_name]:
...
def create_sample_data():
	...
	"distributor_id": "09f3dcc5-505c-4b53-a745-2e7a2f8c195_f",
