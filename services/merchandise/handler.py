...
def get_every_merchandise(event, context):
    """
    Get all merchandise
    """
    ...
    items = context.repo.get_all_merchandises(supplier)
...
@insert_repo
@check_supplier
def modify_merchandise(event, context):
    logger.debug('event: {}'.format(event))
    ...
    try:
        item = context.repo.save_merchandise(body)
...
def get_by_id(event, context):
    ...
    try:
        ...
        item = context.repo.get_merchandise_by_id(context.supplier_id, entity_id)
...
def delete_by_id(event, context):
    """
    Delete merchandise by entity_id and version
    """
    logger.debug('event: {}'.format(event))
    ...
    try:
        ...
        context.repo.delete_merchandise_by_id(context.supplier_id, entity_id)
        return {
            'statusCode': 204
        }
    except NoSuchEntity:
